...
Многие из вас, скорее всего, создавали веб-приложения. И вы хотели поделиться им - с другом, с заказчиком или просто похвастаться. Но для этого надо опубликовать сайт, и с этим могут возникнуть проблемы. В этой статье будет подробно рассказано про то, как **как поставить сайт на VDS**, развернуть сервер и докер-контейнеры.
...
```bash
git clone https://github.com/alexeeev-prog/website-deploy-tutorial
```
